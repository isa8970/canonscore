...
const Detalle = ({ item, onVolver }) => {
  ...
  return (
    <div className="w-full min-h-screen bg-zinc-950 text-white pb-24">
      ...
      <div className="max-w-300 mx-auto px-6 -mt-24 md:-mt-48 relative z-10 grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
        <div className="flex flex-col items-center md:items-start">
          ...
          <div className="mt-6 w-full bg-zinc-900/50 border border-white/5 rounded-2xl p-5">
            ...
            <p className="text-sm opacity-60 mb-2">
              <span className="font-bold text-white">Año:</span> {item.anio}
            </p>
            <p className="text-sm opacity-60 mb-2">
            ...
            <p className="text-sm mb-2">
              <span className="font-bold text-(--accent)">Valoración:</span>{" "}
              {totalValoraciones === 0 ? (
                <span className="text-sm opacity-60">Sin valoraciones</span>
              ) : (
        ...
        <div className="md:col-span-2 text-left flex flex-col justify-start">
          ...
          <h3 className="text-lg font-bold mb-2 border-b border-white/5 pb-2">
            Sinopsis
          </h3>
            ...
            {esInvitado ? (
              ...
              <form onSubmit={deponerComentario} className="mb-8">
                ...
                <div className="flex justify-between items-center mt-1.5 px-1">
                  <span
                    className={`text-[11px] font-mono ${textoValido ? "text-emerald-500" : "text-zinc-600"}`}
                  >
                    {caracteresActuales < MIN_CARACTERES
                ...
                {errorEnvio && (
                  <p className="text-rose-400 text-xs mt-3">{errorEnvio}</p>
                )}
            ...
            {cargandoComentarios ? (
              ...
              <div className="space-y-4">
                {comentarios.map((com) => (
                  <div
                    key={com.id}
                    className="bg-zinc-900/40 border border-white/5 rounded-xl p-4 transition-all"
                  >
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-xs font-bold text-(--accent) flex items-center gap-2">
                        {com.perfiles?.username || "Usuario"}
                        <img
                          src={com.rating ? iconoBuena : iconoMala}
                          alt={
                            com.rating ? "Valoración buena" : "Valoración mala"
                          }
                          className="w-6 h-6 object-contain"
                        />
                      </span>
                      <span className="text-[10px] text-zinc-600 font-mono">
                        {formatearFecha(com.created_at)}
                      </span>
                    </div>
